...
const createProjectCard = (project, index) => {
  ...
  completeBtn.addEventListener("click", (e) => {
    ...
    const confirmed = confirm(
      `${project.isCompleted() ? `Move "${project.name}" back to active projects?` : `Mark "${project.name}" as completed?`}`,
    );
...
const getProcessedProjects = (projects) => {
  ...
  result.sort((a, b) => {
    ...
    if (currentSort === "priority-date") {
      const priorityDiff =
        priorityMap[summaryB.priority] - priorityMap[summaryA.priority];
...
const createTodoItem = (todo) => {
  ...
  checkbox.addEventListener("change", () => {
    ...
    const allComplete =
      todos.length > 0 && todos.every((t) => t.getData().completed);

    if (allComplete && !project.isCompleted()) {
      // Defer so the browser repaints the Done section before the prompt appears
      setTimeout(() => {
        const confirmed = confirm(
          `Looks like that is everything on the todo list — mark "${project.name}" as complete?`,
        );
        if (confirmed) {
          project.toggleCompleted();
          appController.saveProjects();
...
const renderTodos = (projectIndex) => {
  ...
  if (completed.length === 0) {
    ...
    placeholder.textContent =
      "No tasks complete yet — click the checkbox to mark a task as done";
  ...
  for (let cellIndex = 0; cellIndex < totalCells; cellIndex++) {
    ...
    allProjects.forEach((project, index) => {
      ...
      project.getTodos().todos.forEach((todo) => {
        ...
        if (
          dueDate.getFullYear() === displayYear &&
          dueDate.getMonth() === displayMonth &&
          dueDate.getDate() === displayDay
        ) {
    ...
    if (matchingItems.length > 0) {
      ...
      if (matchingItems.length > 1) {
        ...
        upBtn.addEventListener("click", (e) => {
          ...
          calendarOffsets[offsetKey] =
            (calendarOffsets[offsetKey] - 1 + matchingItems.length) %
